...
export const FullscreenPlayer = () => {
  ...
  const recommendationDecision = usePlayerStore((state) => state.recommendationDecision);

  // Guarantee nextTrackItem is never null so the Up Next card is always visible
  const nextTrackItem =
    queue[currentIndex + 1] ||
    recommendationDecision?.track ||
    (queue.length > 1 ? queue.find((t) => t.id !== currentTrack?.id) : null) ||
    MOCK_TRACKS.find((t) => t.id !== currentTrack?.id) ||
    MOCK_TRACKS[0];
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between">
        ...
        {isYouTubeTrack ? (
          ...
          >
            {/* Custom Luxury Cinema Display Icon */}
            <svg
      ...
      <div className="relative z-10 flex flex-col items-center justify-center my-auto max-w-lg mx-auto w-full">
        ...
        {/* Next Song / Up Next Card */}
        {nextTrackItem && (
          <div
            onClick={nextTrack}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') nextTrack(); }}
            aria-label={`Up next: ${nextTrackItem.title} by ${nextTrackItem.artist}`}
            title="Click to play next song"
            className="w-full flex items-center justify-between p-2.5 sm:p-3 rounded-2xl bg-white/[0.04] hover:bg-white/[0.08] text-xs mb-5 border border-white/5 cursor-pointer transition-all duration-300 group shadow-sm hover:border-white/15 active:scale-[0.99]"
          >
            <div className="flex items-center gap-3 min-w-0 pr-2">
              ...
              <div className="min-w-0 text-left">
                <div className="flex items-center gap-1.5">
                  ...
                  {nextTrackItem.recommendationReason && (
                    <span className="text-[10px] text-white/40 truncate hidden sm:inline">
                      • {nextTrackItem.recommendationReason}
                    </span>
                  )}
                </div>
                <p className="text-xs sm:text-sm font-semibold text-white truncate group-hover:text-white transition-colors">
                  {nextTrackItem.title}
                </p>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-[10px] font-mono text-neutral-400">
                {formatDuration(nextTrackItem.duration)}
              </span>
              <div className="w-7 h-7 rounded-full bg-white/10 group-hover:bg-white text-white group-hover:text-black flex items-center justify-center transition-all shadow-sm">
