...
export const useLibraryStore = create((set, get) => ({
  ...
  syncWithBackend: async () => {
    try {
      ...
      if (Array.isArray(remoteFavorites) && remoteFavorites.length > 0) {
        ...
        const fullTracks = remoteFavorites.map((f) => f.track).filter(Boolean);
  ...
  getLikedTracks: () => {
    const { likedTrackIds, likedTracks } = get();
    // Combine tracks stored in memory/MongoDB with fallback mock tracks
    const map = new Map();
    likedTracks.forEach((t) => map.set(t.id, t));
    MOCK_TRACKS.forEach((t) => {
      if (likedTrackIds.includes(t.id) && !map.has(t.id)) {
        map.set(t.id, t);
      }
    });
